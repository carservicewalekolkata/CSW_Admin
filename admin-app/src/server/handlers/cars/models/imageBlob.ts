...
import { downloadModelImage } from '@/lib/azureStorage'
...
export const streamAzureModelImage = async (
  request: NextRequest,
  params: { blob: string[] },
) => {
  ...
  try {
    const result = await downloadModelImage(blobName)
    ...
    console.error('❌ Error streaming Azure model image:', error)
