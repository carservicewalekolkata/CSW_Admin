  ...
  try {
    ...
    if (file.type && !allowedMimeTypes.has(file.type)) {
      return applyCors(
        request,
        versionedJson(
          { success: false, message: 'Unsupported image format. Use PNG, JPEG, WebP, or SVG.' },
          { status: 415 },
        ),
      )
    }
    ...
    if (file.size > MAX_IMAGE_SIZE_BYTES) {
      return applyCors(
        request,
        versionedJson(
          { success: false, message: 'Image must be 4 MB or smaller' },
          { status: 413 },
        ),
      )
    }
    ...
    const derivedBase =
      requestedBase && requestedBase.trim().length > 0
        ? requestedBase
        : (file.name ?? '').split('.').slice(0, -1).join('.') || 'service-image'
