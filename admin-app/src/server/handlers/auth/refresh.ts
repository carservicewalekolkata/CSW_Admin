...
export async function POST(request: NextRequest) {
  ...
  if (!refreshCookie) {
    return versionedJson({ message: 'Refresh token missing' }, { status: 401 })
  }
  ...
  try {
    ...
    return versionedJson({ message: 'Invalid refresh token' }, { status: 401 })
  }
  ...
  if (!user) {
    return versionedJson({ message: 'User not found' }, { status: 401 })
  }

  const tokenVersion = user.refreshTokenVersion ?? 0

  if (payload.tokenVersion !== tokenVersion) {
    return versionedJson({ message: 'Token version mismatch' }, { status: 401 })
  }
